...
fn read_from_ppt(send: Sender<BoardEvent>, ppt_pid: process_memory::Pid) {
    let process_handler: ProcessHandle = match ppt_pid.try_into_process_handle() {
        Ok(handle) => handle,
        Err(_) => {
            send.send(BoardEvent::Exit).unwrap();
            return;
        }
    };
    ...
    while ppt.still_active().unwrap() {
        ...
        if prev_board == Some(bit_board) && prev_hold == hold {
            // let _ = cursor().goto(0, 7);
            // println!("check 1");
            // println!(
            //     "current = {:?}, queue = {:?}, hold = {:?}",
            //     current_piece, queue, hold
            // );
            continue;
        }
    ...
    loop {
        match board_recv.recv().unwrap() {
            BoardEvent::Continue(bit_board, current_piece, queue, hold) => {
                ...
                // && prev_solution[0].shape == current_piece
                {
